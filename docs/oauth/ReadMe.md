...
```
توجه کنید که در این درخواست هم `access_token` و هم `refresh_token` تازه‌سازی می‌شود پس حتما مقادیر جدید را برای استفاده‌های آینده ذخیره کنید. به منظور جلوگیری از مشکلات احتمالی توکن‌ها قدیمی تا دو دقیقه بعد از تازه‌سازی قابل استفاده هستند و در صورت بروز مشکل، مجدد تلاش کنید.
